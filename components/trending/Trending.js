...
const customerid = myData.id
   ...
   const loadscreen = async ()=>{
    ...
    const getList = async()=>{
      await axios.get(`http://3.6.180.23:5000/ecomlist/hy/listofall`,
      {
        headers:{
          'Content-Type':'application/json'
        }}) .then( (response) =>{
          ...
          if(response.status===200){
            setGadgetlist(response.data.request)
            loadscreen()
          }
...
{/* heading*/}
        ...
        <View>
        <Text style={{fontSize:25, fontWeight:'bold', color:'#F83758'}}>trending</Text>
        <Text style={{fontSize:38, fontWeight:'bold', color:'#F83758'}}>24 hours</Text>
      </View>
